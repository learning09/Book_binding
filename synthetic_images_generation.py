...
class perturbed(object):
	...
	def save_img(self, m, n, fold_curve='fold', repeat_time=4, relativeShift_position='relativeShift_v2'):
		...
		for repeat_i in range(repeat_time):
			if fold_curve == 'fold':
				...
				if normalizationFun_0_1:
					alpha_perturbed = random.randint(50, 120) / 100
				else:
					alpha_perturbed = random.randint(60, 120) / 100
			else:
				fold_curve_random = False  # self.is_perform(0.01, 0.99)
				is_normalizationFun_mixture = False  # self.is_perform(0.01, 0.99)
				if normalizationFun_0_1:
					alpha_perturbed = random.randint(60, 140) / 100
				else:
					alpha_perturbed = random.randint(70, 140) / 100
			...
			if fold_curve == 'fold':
				if is_normalizationFun_mixture:
					if self.is_perform(0.5, 0.5):
						...
						# alpha_perturbed = random.randint(60, 120) / 100
						perturbed_d = self.get_0_1_d(np.abs(perturbed_distance_vertex_and_line), random.randint(1, 4))
				else:
					if normalizationFun_0_1:
						# alpha_perturbed = random.randint(40, 120) / 100
						perturbed_d = self.get_0_1_d(np.abs(perturbed_distance_vertex_and_line), random.randint(1, 4))
					else:
						# alpha_perturbed = random.randint(80, 140) / 100
						perturbed_d = np.abs(self.get_normalize(perturbed_distance_vertex_and_line))
				...
				if is_normalizationFun_mixture:
					if self.is_perform(0.5, 0.5):
						...
						# alpha_perturbed = random.randint(90, 110) / 100
						perturbed_d = self.get_0_1_d(np.abs(perturbed_distance_vertex_and_line), random.randint(1, 4))
				else:
					if normalizationFun_0_1:
			...
			'''shadow'''
			if self.is_perform(0.2, 0.8) and repeat_time > 4:
				synthesis_perturbed_img_map[x_min:x_max, y_min:y_max] = np.minimum(np.maximum(synthesis_perturbed_img_map[x_min:x_max, y_min:y_max] - np.int16(np.round(omega_perturbed[x_min:x_max, y_min:y_max].repeat(3).reshape(x_max-x_min, y_max-y_min, 3) * np.linalg.norm(perturbed_v//4)*random.random())), 0), 255)
			''''''
		...
		perspective_shreshold = 320 #520#240
		...
		if fold_curve == 'curve' and self.is_perform(0.1, 0.9):
			while True:
		...
		if is_save_perturbed:
			...
			# synthesis_perturbed_label_clip -= synthesis_perturbed_label_min
			if is_save_perturbed:
				...
				if self.is_perform(0.5, 0.5):
					...
					if self.is_perform(0.2, 0.8) or repeat_time > 5:
					...
					self.synthesis_perturbed_img[foreORbackground_label == 1] = synthesis_perturbed_img_filter[foreORbackground_label == 1]
				...
				with open(self.save_path+'grey/'+perfix_+'_'+fold_curve+'.gw', 'wb') as f:
					...
					f.write(pickle_perturbed_data)
				cv2.imwrite(self.save_path+'outputs/'+perfix_+'_'+fold_curve+'.png', grey)	
				cv2.imwrite(self.save_path+'outputs/'+perfix_+'_'+fold_curve+'.png', synthesis_perturbed_color[:, :, :3])
...
def xgw(args):
	...
	for m, img_path in enumerate(all_img_path):
		for n in range(args.sys_num, args.sys_num_e):
...
if __name__ == '__main__':
	...
	parser.add_argument('--sys_num', default=21, type=int)

	parser.add_argument('--sys_num_e', default=26, type=int)
